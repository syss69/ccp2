...
import userNodel from "../Models/UserSchema.js";
...
class repoMission {
  ...
  async modifyMission(id, data) {
    try {
      ...
      if (!mission)
        return { status: false, message: "mission does not exists" };
  ...
  async getMyMissions(data) {
    try {
      const missions = await userNodel.findById(data).select("missions");
      return { status: true, missions: missions };
    } catch (err) {
      console.error(err.message);
      return { status: false, message: "Internal server error" };
