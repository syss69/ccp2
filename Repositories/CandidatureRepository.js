...
class repoCandidature {
  ...
  async approveCandidature(data) {
    try {
      const candidature = await candidatureModel.findByIdAndUpdate(data, {
        status: "Acceptée",
      });
      if (!candidature) {
      ...
      const mission = await missionModel.findByIdAndUpdate(
        candidature.mission,
        { $addToSet: { members: candidature.participant } },
        { new: true }
      );
      const user = await userModel.findByIdAndUpdate(
        candidature.participant,
        { $addToSet: { missions: candidature.mission } },
        { new: true }
      );
      if (!mission || !user) {
  ...
  async getAllForMission(data) {
    try {
      ...
      if (candidatures.length === 0) {
        return {
          status: true,
          message: "No candidatures for this mission! Yet...",
        };
      }
